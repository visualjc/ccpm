...
### 1. Validation Script
```bash
./.cursor/ccpm/scripts/check-path-standards.sh
```
...
### 2. Fix Script  
```bash
./.cursor/ccpm/scripts/fix-path-standards.sh
```
...
- name: Path Standards Check
  run: ./.cursor/ccpm/scripts/check-path-standards.sh
```

### Clean Up Backups
After confirming fixes are correct:
```bash
find .cursor/ccpm/ -name '*.backup' -delete
```

## Standards Reference
For detailed path usage guidelines, see: `.cursor/ccpm/rules/path-standards.md`
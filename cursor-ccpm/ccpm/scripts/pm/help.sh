...
echo "📚 Cursor CCPM - Project Management System"
# Try .cursor/ccpm/VERSION first (installed location), then claude-template/VERSION (dev location)
if [[ -f ".cursor/ccpm/VERSION" ]]; then
  VERSION=$(cat .cursor/ccpm/VERSION)
...
echo ""
echo "📁 Planning Storage"
echo "  • New repos store planning artifacts under docs/prds/<prd>/..."
echo "  • Config stays in .claude/.ccpmrc"
echo "  • Migrate legacy .claude/prds + .claude/epics with /pm:migrate-layout"
echo ""
echo "📄 PRD Commands"
...
echo "  /pm:migrate-layout     - Preview or apply nested planning storage migration"
echo ""
echo "💡 Tips"
...
echo "  • New PRDs are stored at docs/prds/<name>/prd.md by default"

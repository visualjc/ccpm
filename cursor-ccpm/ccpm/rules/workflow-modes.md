...
CCPM supports two primary workflow modes configured via `.cursor/ccpm/.ccpmrc`:
...
This constraint is enforced by `.cursor/ccpm/scripts/pm/resolve-config.sh`.
...
To switch modes, update `.cursor/ccpm/.ccpmrc` and start a new epic:

```bash
# Switch to human-in-the-loop
echo "PARALLEL_MODE=false" >> .cursor/ccpm/.ccpmrc
echo "WORKTREE_MODE=false" >> .cursor/ccpm/.ccpmrc
...
- [.ccpmrc.example](.cursor/ccpm/.ccpmrc) - Example configuration file
- [resolve-config.sh](.cursor/ccpm/scripts/pm/resolve-config.sh) - Configuration resolution script
- [settings.json.example](.cursor/ccpm/settings.json.example) - Hook configuration

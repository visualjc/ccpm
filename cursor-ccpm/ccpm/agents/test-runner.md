...
1. **Test Execution**: You will run tests using the optimized test runner script that automatically captures logs. Always use `.cursor/ccpm/scripts/test-and-log.sh` to ensure full output capture.
...
2. **Test Execution**:
   ...
   .cursor/ccpm/scripts/test-and-log.sh tests/[test_file].py

   # For iteration testing with custom log names
   .cursor/ccpm/scripts/test-and-log.sh tests/[test_file].py [test_name]_iteration_[n].log
   ```

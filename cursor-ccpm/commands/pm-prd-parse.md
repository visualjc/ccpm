...
2. **Verify PRD exists:**
   - Resolve PRD directory: `PRD_DIR=$(.cursor/ccpm/scripts/pm/resolve-prd-dir.sh)`
...
3. **Validate PRD frontmatter:**
   ...
   - If frontmatter is invalid or missing, tell user: "❌ Invalid PRD frontmatter. Please check: .cursor/ccpm/prds/$ARGUMENTS.md"
   - Show what's missing or invalid

4. **Check for existing epic:**
   - Check if `.cursor/ccpm/epics/$ARGUMENTS/epic.md` already exists
...
5. **Verify directory permissions:**
   - Ensure `.cursor/ccpm/epics/` directory exists or can be created
...
Create the epic file at: `.cursor/ccpm/epics/$ARGUMENTS/epic.md` with this exact structure:
...
- `.cursor/ccpm/epics/$ARGUMENTS/` (directory)
- `.cursor/ccpm/epics/$ARGUMENTS/epic.md` (epic file)
...
1. Confirm: "✅ Epic created: .cursor/ccpm/epics/$ARGUMENTS/epic.md"
2. Show summary of:

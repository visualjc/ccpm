...
1. **Verify epic exists:**
   - Check if `.cursor/ccpm/epics/$ARGUMENTS/epic.md` exists
   - If not found, tell user: "❌ Epic not found: $ARGUMENTS. First create it with: /pm:prd-parse $ARGUMENTS"
   - Stop execution if epic doesn't exist

2. **Check for existing tasks:**
   - Check if any numbered task files (001.md, 002.md, etc.) already exist in `.cursor/ccpm/epics/$ARGUMENTS/`
...
3. **Validate epic frontmatter:**
   ...
   - If invalid, tell user: "❌ Invalid epic frontmatter. Please check: .cursor/ccpm/epics/$ARGUMENTS/epic.md"
...
### 1. Read the Epic
- Load the epic from `.cursor/ccpm/epics/$ARGUMENTS/epic.md`
...
Task:
  ...
  prompt: |
    ...
    For each task:
    1. Create file: .cursor/ccpm/epics/$ARGUMENTS/{number}.md
...
1. **For Each Task in Epic Breakdown:**
   - Create task file: `.cursor/ccpm/epics/$ARGUMENTS/{number}.md`
...
Save tasks as: `.cursor/ccpm/epics/$ARGUMENTS/{task_number}.md`

...
```bash
PRD_PATH=$(.cursor/ccpm/scripts/pm/resolve-prd-path.sh "$ARGUMENTS") || { echo "❌ PRD not found. Run /pm:prd-new $ARGUMENTS first"; exit 1; }
```
Read `"$PRD_PATH"`:
- Parse frontmatter
- Read all sections
...
Follow `.cursor/ccpm/rules/frontmatter-operations.md`.

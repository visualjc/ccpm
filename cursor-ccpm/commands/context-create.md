...
This command creates the initial project context documentation in `.cursor/ccpm/context/` by analyzing the current project state and establishing comprehensive baseline documentation.
...
- Run: `ls -la .cursor/ccpm/context/ 2>/dev/null`
- If directory exists and has files:
  - Count existing files: `ls -1 .cursor/ccpm/context/*.md 2>/dev/null | wc -l`
...
- If `.cursor/ccpm/context/` doesn't exist, create it: `mkdir -p .cursor/ccpm/context/`
- Verify write permissions: `touch .cursor/ccpm/context/.test && rm .cursor/ccpm/context/.test`
...
**Common Issues:**
- **No write permissions:** "❌ Cannot write to .cursor/ccpm/context/. Check permissions."
...
📁 Created context in: .cursor/ccpm/context/
...
- Target directory: `.cursor/ccpm/context/` (create if needed)

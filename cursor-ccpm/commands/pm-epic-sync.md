...
# Count task files
ls .cursor/ccpm/epics/$ARGUMENTS/*.md 2>/dev/null | grep -v epic.md | wc -l
```
...
Follow `.cursor/ccpm/rules/github-operations.md` to ensure we're not syncing to the CCPM template:
...
sed '1,/^---$/d; 1,/^---$/d' .cursor/ccpm/epics/$ARGUMENTS/epic.md > /tmp/epic-body-raw.md
...
```bash
PARALLEL_MODE=$(.cursor/ccpm/scripts/pm/resolve-config.sh PARALLEL_MODE)
WORKTREE_MODE=$(.cursor/ccpm/scripts/pm/resolve-config.sh WORKTREE_MODE)
...
task_count=$(ls .cursor/ccpm/epics/$ARGUMENTS/[0-9][0-9][0-9].md 2>/dev/null | wc -l)
```
...
if [ "$task_count" -lt 5 ] || [ "$PARALLEL_MODE" = "false" ]; then
  for task_file in .cursor/ccpm/epics/$ARGUMENTS/[0-9][0-9][0-9].md; do
...
sed -i.bak "/^github:/c\github: $epic_url" .cursor/ccpm/epics/$ARGUMENTS/epic.md
sed -i.bak "/^updated:/c\updated: $current_date" .cursor/ccpm/epics/$ARGUMENTS/epic.md
rm .cursor/ccpm/epics/$ARGUMENTS/epic.md.bak
```
...
for task_file in .cursor/ccpm/epics/$ARGUMENTS/[0-9]*.md; do
  [ -f "$task_file" ] || continue
...
total_count=$(ls .cursor/ccpm/epics/$ARGUMENTS/[0-9]*.md 2>/dev/null | wc -l)
parallel_count=$(grep -l '^parallel: true' .cursor/ccpm/epics/$ARGUMENTS/[0-9]*.md 2>/dev/null | wc -l)
...
cp .cursor/ccpm/epics/$ARGUMENTS/epic.md .cursor/ccpm/epics/$ARGUMENTS/epic.md.backup
...
' .cursor/ccpm/epics/$ARGUMENTS/epic.md.backup > .cursor/ccpm/epics/$ARGUMENTS/epic.md

# Clean up
rm .cursor/ccpm/epics/$ARGUMENTS/epic.md.backup
rm /tmp/tasks-section.md
```

### 6. Create Mapping File

Create `.cursor/ccpm/epics/$ARGUMENTS/github-mapping.md`:
```bash
# Create mapping file
cat > .cursor/ccpm/epics/$ARGUMENTS/github-mapping.md << EOF
...
for task_file in .cursor/ccpm/epics/$ARGUMENTS/[0-9]*.md; do
  ...
  echo "- #${issue_num}: ${task_name} - https://github.com/${repo}/issues/${issue_num}" >> .cursor/ccpm/epics/$ARGUMENTS/github-mapping.md
done

# Add sync timestamp
echo "" >> .cursor/ccpm/epics/$ARGUMENTS/github-mapping.md
echo "Synced: $(date -u +"%Y-%m-%dT%H:%M:%SZ")" >> .cursor/ccpm/epics/$ARGUMENTS/github-mapping.md
```
...
Follow `.cursor/ccpm/rules/worktree-operations.md` to create development worktree:
...
Follow `.cursor/ccpm/rules/github-operations.md` for GitHub CLI errors.

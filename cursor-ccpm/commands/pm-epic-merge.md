...
2. **Check for active agents:**
   Read `.cursor/ccpm/epics/$ARGUMENTS/execution-status.md`
...
Update `.cursor/ccpm/epics/$ARGUMENTS/epic.md`:
...
feature_list=""
if [ -d ".cursor/ccpm/epics/$ARGUMENTS" ]; then
  cd .cursor/ccpm/epics/$ARGUMENTS
...
epic_github_line=$(grep 'github:' .cursor/ccpm/epics/$ARGUMENTS/epic.md 2>/dev/null || true)
...
mv .cursor/ccpm/epics/$ARGUMENTS .cursor/ccpm/epics/archived/
echo "✅ Epic archived: .cursor/ccpm/epics/archived/$ARGUMENTS"
```
...
# Extract epic issue number
epic_github_line=$(grep 'github:' .cursor/ccpm/epics/archived/$ARGUMENTS/epic.md 2>/dev/null || true)
if [ -n "$epic_github_line" ]; then
  epic_issue=$(echo "$epic_github_line" | grep -oE '[0-9]+$' || true)
else
  epic_issue=""
...
# Close task issues
for task_file in .cursor/ccpm/epics/archived/$ARGUMENTS/[0-9]*.md; do
  [ -f "$task_file" ] || continue

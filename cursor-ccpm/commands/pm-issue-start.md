...
2. **Find local task file:**
   - First check if `.cursor/ccpm/epics/*/$ARGUMENTS.md` exists (new naming)
   - If not found, search for file containing `github:.*issues/$ARGUMENTS` in frontmatter (old naming)
   - If not found: "❌ No local task for issue #$ARGUMENTS. This issue may have been created outside the PM system."

3. **Check for analysis:**
   ```bash
   test -f .cursor/ccpm/epics/*/$ARGUMENTS-analysis.md || echo "❌ No analysis found for issue #$ARGUMENTS
...
Read `.cursor/ccpm/epics/{epic_name}/$ARGUMENTS-analysis.md`:
...
```bash
mkdir -p .cursor/ccpm/epics/{epic_name}/updates/$ARGUMENTS
```
...
Create `.cursor/ccpm/epics/{epic_name}/updates/$ARGUMENTS/stream-{X}.md`:
```markdown
---
issue: $ARGUMENTS
stream: {stream_name}
...
Task:
  ...
  prompt: |
    ...
    Requirements:
    1. Read full task from: .cursor/ccpm/epics/{epic_name}/{task_file}
    2. Work ONLY in your assigned files
    3. Commit frequently with format: "Issue #$ARGUMENTS: {specific change}"
    4. Update progress in: .cursor/ccpm/epics/{epic_name}/updates/$ARGUMENTS/stream-{X}.md
    5. Follow coordination rules in .cursor/ccpm/rules/agent-coordination.md
...
Progress tracking:
  .cursor/ccpm/epics/{epic_name}/updates/$ARGUMENTS/
...
Follow `.cursor/ccpm/rules/datetime.md` for timestamps.

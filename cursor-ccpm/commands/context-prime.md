...
- If directory doesn't exist or is empty:
  ...
  - Exit gracefully
- Count available context files: `ls -1 .cursor/ccpm/context/*.md 2>/dev/null | wc -l`
- Report: "📁 Found {count} context files to load"

### 2. File Integrity Check
- For each context file found:
  - Verify file is readable: `test -r ".cursor/ccpm/context/{file}" && echo "readable"`
  - Check file has content: `test -s ".cursor/ccpm/context/{file}" && echo "has content"`

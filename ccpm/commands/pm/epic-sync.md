...
1. Resolve the epic directory:
   ...
   ```
2. Use `"$EPIC_DIR/issues/"` as the canonical task directory. Only fall back to legacy task files in `"$EPIC_DIR/"` if `issues/` is absent.
...
9. Confirm with epic issue number, task count, and worktree path.

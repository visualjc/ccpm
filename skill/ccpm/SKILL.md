---
name: ccpm
description: "CCPM - spec-driven project management: PRD → Epic → GitHub Issues → parallel agents → shipped code. Use this skill for anything in the software delivery lifecycle: writing a PRD ('write a PRD for X', 'let's plan X', 'scope this out'), parsing a PRD into an epic, decomposing an epic into tasks, syncing to GitHub ('sync the X epic', 'push tasks to github'), starting work on an issue ('start working on issue N', 'let's work on issue N'), analyzing parallel work streams, running standups ('standup', 'run the standup'), checking status ('what's next', 'what's blocked', 'what are we working on'), creating or refreshing project context, or priming and running tests as part of delivery. Use ccpm any time the user is talking about shipping a feature, managing work, maintaining project context, or validating implementation progress — even if they don't say 'ccpm' or 'PRD'. Do NOT use for: raw GitHub issue/PR operations with no delivery context, unrelated code review, or ad hoc debugging with no project-management intent."
---
...
**When**: User wants to create initial project context, refresh existing context, or prime a new session with project knowledge.
**Read**: `references/context.md`
**Covers**: Context creation, targeted context updates, and context priming from `.claude/context/`.

### Testing — Prime and run validation
**When**: User wants to detect the test setup, create testing config, run tests, or analyze failures in the context of delivery work.
**Read**: `references/testing.md`
**Covers**: Test framework detection, `.claude/testing-config.md`, targeted test execution, and failure reporting.
...
Create context:     "create project context"
Refresh context:    "update project context"

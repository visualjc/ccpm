...
if [ -n "$recent_files" ]; then
  # Count by type
  prd_count=$(echo "$recent_files" | grep -c "/prds/" 2>/dev/null)
  prd_count=${prd_count:-0}
  epic_count=$(echo "$recent_files" | grep -c "/epic.md" 2>/dev/null)
  epic_count=${epic_count:-0}
  task_count=$(echo "$recent_files" | grep -c "/[0-9]*.md" 2>/dev/null)
  task_count=${task_count:-0}
  update_count=$(echo "$recent_files" | grep -c "/updates/" 2>/dev/null)
